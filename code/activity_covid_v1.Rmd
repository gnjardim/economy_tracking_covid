...
brasil_df <- brasil %>% 
    mutate(doubl_dates = c(NA, doubl_dates) %>% as_date(),
           ...
           mobility    = (100 + ((retail_and_recreation_percent_change_from_baseline + 
                          grocery_and_pharmacy_percent_change_from_baseline) / 2 +  
                              workplaces_percent_change_from_baseline) / 2) / 100,
           activity    = ifelse(relation_eq(mobility) > 1, 
                                1, relation_eq(mobility)),
           smth_date   = rollmean(dist_date, 7, na.pad = TRUE),
           smth_activ  = rollmean(activity, 7, na.pad = TRUE) * 100
          )
