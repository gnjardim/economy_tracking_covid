...
brasil_df <- brasil %>% 
    mutate(doubl_dates = c(NA, doubl_dates) %>% as_date(),
           ...
           smth_activ  = rollmean(activity, 7, na.pad = T, align = "right")
          )
...
pre_covid_df_fun <- function(df) {
    ...
    post_df <- df %>% 
    filter(data >= as.Date("2019-01-01") & data <= as.Date("2020-02-25")) %>% 
    group_by(data, uf, estado) %>% 
    summarise(consumo_diario_pre = sum(consumo)) %>% 
    group_by(uf, estado) %>% 
    summarise(media_diaria_pre = mean(consumo_diario_pre))
...
energy_df_fun <- function(df, df_pre_cov) {
    ...
    post_df <- df %>% 
        ...
        mutate(dif_baseline = (consumo - media_diaria_pre) / media_diaria_pre) %>% 
