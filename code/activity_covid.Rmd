...
find_initial_date <- function(df) {
    
    initial_date <- df %>% 
        filter(!is.na(smth_date)) %>% 
        slice(1) %>% 
        pull(date)
}

find_end_date <- function(df, activity_column) {
    
    end_date <- df %>% 
        filter(!is.na( !!activity_column )) %>% 
        slice(n()) %>% 
        pull(date)
}
...
### define dates
initial_date <- find_initial_date(brasil)
end_date     <- find_end_date(brasil_df, "mobility")
dates <- c(initial_date, end_date)

brasil_df <- brasil_df %>% 
    mutate(inicio_fim = ifelse(date %in% dates, date, NA) %>% as.Date())
```
...
ggplot(brasil_df, aes(x = smth_date, y = smth_mob)) +
    ...
    geom_text(aes(label = inicio_fim)) +
    xlim(0, 40) + 
...
pre_covid_df_fun <- function(df) {
    ...
    mod <- lm(consumo_diario ~ factor(estado) * factor(mes) + factor(ano), 
              data = df_oss)    
...
energy_df_fun <- function(df, df_pre_cov) {
    ...
    post_df <- df %>% 
        ...
        group_by(estado) %>% 
        arrange(estado, data) %>% 
        mutate(smth_consumo = pmin(0, rollmean(dif_baseline, 7, na.pad = T, align = "right")))
...
end_date <- find_end_date(brasil_df, "dif_baseline")
```
